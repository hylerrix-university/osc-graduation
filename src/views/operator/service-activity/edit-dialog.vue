      ...
      <v-card-text>
        <v-container grid-list-md>
          <v-layout wrap>
            ...
            <v-flex xs12 md6>
              <v-text-field v-model="editedItem.time" label="举办时间"></v-text-field>
            </v-flex>
            <v-flex xs12 md6>
              <v-text-field v-model="editedItem.status" label="活动状态"></v-text-field>
            </v-flex>
            <v-flex xs12 md6>
              <v-text-field v-model="editedItem.remark" label="活动备注"></v-text-field>
            </v-flex>
          </v-layout>
        </v-container>
      </v-card-text>
...
<script lang='ts'>
  ...
  export default class EditDialog extends Vue {
    ...
    get title() {
      return this.editedIndex === -1 ? '添加活动' : '编辑活动'
    }
